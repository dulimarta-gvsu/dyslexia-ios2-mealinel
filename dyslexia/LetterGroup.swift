    ...
    var body: some View {
        ZStack {
            ...
            if let draggedLetter {
                BigLetter(letter: draggedLetter, size: letterSize, backgroundColor: letterColor)
                    .offset(
                ...
                HStack(spacing: 2) {
                    if letters.count > 0 {
                        ForEach(Array(self.letters.enumerated()), id: \.offset) { _, letter in
                            BigLetter(letter: letter, size: letterSize, backgroundColor: letterColor)
                        }
                    } else {
                        BigLetter(letter: Letter(), size: letterSize, backgroundColor: letterColor)
                    }
...
struct BigLetter: View {
    ...
    var body: some View {
        ZStack(alignment: .bottomTrailing) {
            ...
            if pt > 0 {
                Text("\(pt)")
                    .font(.system(size: 0.18 * self.size, weight: .bold))
                    .padding(4)
            }
        }
